...
public class PairingManagerJava {
    ...
    public PairingManagerJava(NvHTTP http, LimelightCryptoProvider cryptoProvider) {
        this.http = http;
    ...
    private static boolean verifySignature(byte[] data, byte[] signature, Certificate cert) {
        try {
            Signature sig = PairingManagerJava.getSha256SignatureInstanceForKey(cert.getPublicKey());
    ...
    private static byte[] signData(byte[] data, PrivateKey key) {
        try {
            Signature sig = PairingManagerJava.getSha256SignatureInstanceForKey(key);
            sig.initSign(key);
            sig.update(data);
            return sig.sign();
        } catch (NoSuchAlgorithmException | SignatureException | InvalidKeyException e) {
    ...
    public PairState pair(ServerInfo serverInfo, String pin, String passphrase) throws IOException, XmlPullParserException {
        PairingHashAlgorithm hashAlgo;

        int serverMajorVersion = serverInfo.getServerMajorVersion();
